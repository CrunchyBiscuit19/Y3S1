...
Planet savedPlanetList[numPlanets];
Planet squareList[numSquares];
Planet savedSquareList[numSquares];
Planet starList[numStars];
Planet savedStarList[numStars];
...
void drawRevolver(float radius, const GLfloat color[3], float alpha) {
    GLfloat chamberColor[3] = {color[0] * 0.3f, color[1] * 0.3f,
                               color[2] * 0.3f};
    ...
    for (int i = 0; i < 6; i++) {
        ...
        glPushMatrix();
        glTranslatef(radius * 0.55f * cos(angle), radius * 0.55f * sin(angle),
                     0);
...
void drawKnockedOutFace(float radius, const GLfloat color[3], float alpha) {
    ...
    drawArc(radius * 0.42f, radius * 0.12f, PI / 9, 8 * PI / 9, featureColor,
            alpha);
    glPopMatrix();
}
...
void drawSmilingFace(float radius, const GLfloat color[3], float alpha) {
    ...
    drawArc(radius * 0.45f, radius * 0.11f, PI + PI / 6, 2 * PI - PI / 6,
            featureColor, alpha);
    glPopMatrix();
}
...
void generateSquares() {
    ...
    GLfloat palette[paletteSize][3] = {{1.000f, 0.992f, 0.745f},
...
void drawFaceSystem(const Planet& face) {
    ...
    glPushMatrix();
    glRotatef(12.0f * sin(face.spin * PI / 180), 0, 0, 1); // head roll
    drawPlanet(face);
    glPopMatrix();
    ...
    for (int i = 0; i < numStars; i++) {
        ...
        glPushMatrix();
        glTranslatef(star.distFromRef * -sin(starAngle),
                     star.distFromRef * cos(starAngle), 0);
...
void drawAngelSystem(const Planet& angel) {
    ...
    drawHalo(angel.size * 0.70f, angel.size * 0.26f, angel.size * 0.09f,
             haloColor, angel.alpha);
    glPopMatrix();
...
void generateStars() {
    const int paletteSize = 3;
    GLfloat palette[paletteSize][3] = {{1.000f, 0.960f, 0.600f},
...
void updateStars() {
    ...
    for (int i = 0; i < numStars; i++) {
        ...
        starList[i].angle -=
            starList[i].angularSpeed * speedFactor * timer;  // clockwise
...
void keyboard(unsigned char key, int x, int y) {
    ...
    switch (key) {
        ...
        case 't':
            ...
            if (clockMode) {
                for (int i = 0; i < numPlanets; i++)
                    savedPlanetList[i] = planetList[i];
                for (int i = 0; i < numSquares; i++)
                    savedSquareList[i] = squareList[i];
                for (int i = 0; i < numStars; i++)
                    savedStarList[i] = starList[i];
                std::cout << "Current Mode: Clock mode." << std::endl;
            } else {
                for (int i = 0; i < numPlanets; i++)
                    planetList[i] = savedPlanetList[i];
                for (int i = 0; i < numSquares; i++)
                    squareList[i] = savedSquareList[i];
                for (int i = 0; i < numStars; i++)
                    starList[i] = savedStarList[i];
